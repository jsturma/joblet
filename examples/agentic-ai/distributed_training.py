...
class DistributedTrainingCoordinator:
    ...
    def synchronize_workers(self, epoch_results):
        ...
        self.logger.info(
            f"Synchronization completed - Global Loss: {global_loss:.4f}, Global Accuracy: {global_accuracy:.4f}")
    ...
    def _save_results(self):
        ...
        training_results = {
            ...
            'model_artifacts': {
                'final_loss': self.global_metrics['global_loss_history'][-1] if self.global_metrics[
                    'global_loss_history'] else None,
                'final_accuracy': self.global_metrics['global_accuracy_history'][-1] if self.global_metrics[
                    'global_accuracy_history'] else None,
            ...
            'performance_metrics': {
                ...
                'average_sync_time': sum(self.global_metrics['synchronization_overhead']) / len(
                    self.global_metrics['synchronization_overhead']) if self.global_metrics[
                    'synchronization_overhead'] else 0,
        ...
        if os.path.exists(self.output_dir):
            results_file = os.path.join(self.output_dir,
                                        f'distributed_training_{self.global_metrics["training_id"][:8]}.json')
