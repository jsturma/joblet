#!/bin/bash
# Self-contained Ubuntu ARM64 Python 3.11 ML Runtime Setup
...
install_python_ml_packages() {
    ...
    #apt-get update - REMOVED: use copy-only approach -qq || echo "⚠ Package update failed, continuing..."
    
    # Install Python and essential packages
    echo "Installing Python 3.11 and development packages..."
    #apt-get install - REMOVED: use copy-only approach -y python3.11 python3.11-dev python3.11-venv python3-pip || {
        echo "⚠ Python 3.11 failed, trying Python 3..."
        #apt-get install - REMOVED: use copy-only approach -y python3 python3-dev python3-venv python3-pip || {
            echo "⚠ Some Python packages failed to install, continuing..."
        }
    }
    
    # Install ML packages using system package manager (more reliable)
    echo "Installing ML packages via APT..."
    #apt-get install - REMOVED: use copy-only approach -y python3-numpy python3-scipy python3-pandas python3-matplotlib \
                       python3-sklearn python3-requests python3-urllib3 || {
        echo "⚠ Some ML packages failed via APT, will try pip as fallback..."
        #pip3 install - REMOVED: use copy-only approach --break-system-packages numpy scipy pandas matplotlib scikit-learn requests urllib3 2>/dev/null || {
            echo "⚠ Pip installation also failed, continuing with basic Python..."
        }
    }
...
copy_essential_libraries() {
    ...
    local lib_dirs=("/usr/lib/aarch64-linux-gnu" "/lib/aarch64-linux-gnu")
...
generate_runtime_yml() {
    ...
    echo "Creating design-compliant runtime.yml for Ubuntu ARM64..."
    cat > "$RUNTIME_YML" << EOF
name: python-3.11-ml
...
environment:
  ...
  LD_LIBRARY_PATH: "/usr/lib/aarch64-linux-gnu:/lib/aarch64-linux-gnu:/lib64:/usr/lib:/lib"
EOF
