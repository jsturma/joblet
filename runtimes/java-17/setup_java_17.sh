...
# Multi-Architecture OpenJDK 17 LTS Runtime Setup Script
...
echo "☕ Setting up Multi-Architecture OpenJDK 17 LTS Runtime"
echo "======================================================="
...
main() {
    ...
    echo "🔍 Getting Java 17 download URL for $DETECTED_ARCH..."
    JAVA_URL=$(get_java_download_url 17)
    if [[ $? -ne 0 ]]; then
    ...
    echo "📦 Java 17 URL: $JAVA_URL"
    
    # Download and install OpenJDK 17
    echo "⬇️  Downloading OpenJDK 17 for $DETECTED_ARCH..."
    download_file "$JAVA_URL" "openjdk-17.tar.gz"
    ...
    # Clean up temporary download tools if we installed them
    if [[ "${JOBLET_CLEANUP_WGET:-}" == "true" ]]; then
        echo "🧹 Cleaning up temporary wget installation..."
        remove_packages "wget" 2>/dev/null || true
    fi
    if [[ "${JOBLET_CLEANUP_CURL:-}" == "true" ]]; then
        echo "🧹 Cleaning up temporary curl installation..."
        remove_packages "curl" 2>/dev/null || true
    fi
    ...
    # Create runtime manifest with system information and mount configurations
...
EOF
    ...
    echo ""
    echo "🎉 OpenJDK 17 Runtime Installation Complete!"
    echo "=============================================="
    ...
    echo "   rnx runtime info java:17"
    echo "   rnx run --runtime=java:17 java -version"
    echo ""
    echo "📚 Example usage:"
    echo "   cd /opt/joblet/examples/java-17"
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    echo "Multi-Architecture OpenJDK 17 LTS Runtime Setup"
    echo ""
    echo "Usage: sudo $0"
    echo ""
    show_supported_platforms
