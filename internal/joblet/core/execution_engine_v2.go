...
// ExecutionEngineV2 is the refactored execution engine using the coordinator pattern
// This replaces the deprecated ExecutionEngine
...
// NewExecutionEngineV2 creates a new execution engine using the coordinator pattern
func NewExecutionEngineV2(
	...
	// Create runtime manager if enabled
...
// StartProcess starts a job process using the new coordinator pattern
...
// StartProcessWithUploads provides backward compatibility
...
func (pma *processManagerAdapter) LaunchProcess(ctx context.Context, config *execution.LaunchConfig) (*execution.ProcessResult, error) {
	...
	// Use the job isolation's proper namespace isolation setup
	// This includes PID, Mount, UTS, IPC, and Network namespace isolation
	sysProcAttr := pma.isolation.CreateIsolatedSysProcAttr()
	...
	procConfig := &process.LaunchConfig{
		...
		Args:        config.Args,
		SysProcAttr: sysProcAttr, // Enable full isolation
	}
